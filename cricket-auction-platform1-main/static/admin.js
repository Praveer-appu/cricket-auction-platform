...
async function api(url, options = {}) {
    ...
    // Don't force HTTPS conversion - use whatever protocol is provided
...
function updatePlayerStats() {
    ...
    const unsold = allAdminPlayers.filter(p => (p.status || '').toLowerCase() === 'unsold').length;
...
function getFilteredAdminPlayers() {
    ...
    return allAdminPlayers.filter(player => {
        ...
        const matchStatus = !filterStatus || (playerStatus === filterStatus);
